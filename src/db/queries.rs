...
pub async fn set_coc_channel(db: &SqlitePool, guild_id: &str, channel_id: Option<&str>) -> Result<()> {
    sqlx::query(
        "INSERT INTO guild_config (guild_id, coc_channel_id) VALUES (?, ?) \
         ON CONFLICT(guild_id) DO UPDATE SET coc_channel_id = excluded.coc_channel_id",
    )
    .bind(guild_id)
    .bind(channel_id)
    .execute(db)
...
pub async fn set_okxxx_channel(
    db: &SqlitePool,
    guild_id: &str,
    channel_id: Option<&str>,
) -> Result<()> {
    sqlx::query(
        "INSERT INTO guild_config (guild_id, okxxx_channel_id) VALUES (?, ?) \
         ON CONFLICT(guild_id) DO UPDATE SET okxxx_channel_id = excluded.okxxx_channel_id",
    )
    .bind(guild_id)
    .bind(channel_id)
    .execute(db)
...
pub async fn is_okxxx_seen(db: &SqlitePool, guild_id: &str, video_id: &str) -> Result<bool> {
    let row = sqlx::query(
        "SELECT 1 FROM seen_okxxx WHERE guild_id = ? AND video_id = ? LIMIT 1",
    )
    .bind(guild_id)
    .bind(video_id)
    .fetch_optional(db)
...
pub async fn mark_okxxx_seen(db: &SqlitePool, guild_id: &str, video_id: &str) -> Result<()> {
    sqlx::query(
        "INSERT OR IGNORE INTO seen_okxxx (guild_id, video_id) VALUES (?, ?)",
    )
    .bind(guild_id)
    .bind(video_id)
    .execute(db)
...
pub async fn prune_old_seen_okxxx(db: &SqlitePool, days: i64) -> Result<u64> {
    let result = sqlx::query(
        "DELETE FROM seen_okxxx WHERE seen_at < datetime('now', ? || ' days')",
    )
    .bind(format!("-{}", days))
    .execute(db)
    .await?;
...
pub async fn set_xnxx_channel(db: &SqlitePool, guild_id: &str, channel_id: Option<&str>) -> Result<()> {
    sqlx::query(
        "INSERT INTO guild_config (guild_id, xnxx_channel_id) VALUES (?, ?) \
         ON CONFLICT(guild_id) DO UPDATE SET xnxx_channel_id = excluded.xnxx_channel_id",
    )
    .bind(guild_id)
    .bind(channel_id)
    .execute(db)
...
pub async fn is_xnxx_seen(db: &SqlitePool, guild_id: &str, item_id: &str) -> Result<bool> {
    let row = sqlx::query(
        "SELECT 1 FROM seen_xnxx WHERE guild_id = ? AND item_id = ? LIMIT 1",
    )
    .bind(guild_id)
    .bind(item_id)
    .fetch_optional(db)
...
pub async fn mark_xnxx_seen(db: &SqlitePool, guild_id: &str, item_id: &str) -> Result<()> {
    sqlx::query(
        "INSERT OR IGNORE INTO seen_xnxx (guild_id, item_id) VALUES (?, ?)",
    )
    .bind(guild_id)
    .bind(item_id)
    .execute(db)
...
pub async fn prune_old_seen_xnxx(db: &SqlitePool, days: i64) -> Result<u64> {
    let result = sqlx::query(
        "DELETE FROM seen_xnxx WHERE seen_at < strftime('%s','now') - ? * 86400",
    )
    .bind(days)
    .execute(db)
    .await?;
...
pub async fn mark_coc_seen(db: &SqlitePool, guild_id: &str, item_id: &str) -> Result<()> {
    sqlx::query(
        "INSERT OR IGNORE INTO seen_coc (guild_id, item_id) VALUES (?, ?)",
    )
    .bind(guild_id)
    .bind(item_id)
    .execute(db)
...
pub async fn prune_old_seen_coc(db: &SqlitePool, days: i64) -> Result<()> {
    sqlx::query(
        "DELETE FROM seen_coc WHERE seen_at < strftime('%s','now') - ?",
    )
    .bind(days * 86_400)
    .execute(db)
    .await?;
